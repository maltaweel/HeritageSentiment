...
def main() -> None:
    ...
    pn=os.path.abspath(__file__)
    pn=pn.split("src")[0]
    fileOutput=os.path.join(pn,"sentiment","sentiment_classification.csv")
    file_read=os.path.join(pn,'modified',"modified_totalTweets.csv")
        
    fieldnames = ['Datetime','ID','Sentiment_Class','Link','Text','Username','Retweets','Hashtags','Geolocation']
        ...
        with open(file_read,'r') as csvfile:
            ...
            for row in reader:
                ...
                for method in args.method:
                    ...
                    row['Sentiment_Class']=exp.top_labels[0]+1
                    output(writer,row)
                    i+=1
...
def output(writer,f):
        ...
        writer.writerow({'Datetime': str(f['Datetime']),
            'ID':str(f['ID']),'Sentiment_Class':str(f['Sentiment_Class']),'Link':str(f['Link']),
